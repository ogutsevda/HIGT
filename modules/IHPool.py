...
class IHPool(torch.nn.Module):
    ...
    def forward(
        self, x, edge_index, node_type, tree, x_y_index, edge_weight=None, batch=None
    ):
        r"""
        x : node feature;
        edge_index :  edge;
        ...
        x_y_fitness_1 = torch.cat(
            (x_y_index[torch.where(node_type == 1)], fitness_1.unsqueeze(1)), -1
        ).to(device)
        ...
        if self.select == "inter":
            if self.ratio < 1:
                index_of_threshold_fitness_1 = sort_fitness_1_index[
                    range(
                        0, N_1, int(torch.ceil(torch.tensor(N_1 / (N_1 * self.ratio))))
                    )
                ].to(device)
            else:
                if N_1 < self.ratio:
        ...
        if self.dis == "ou":
            cosine_dis_1 = euclidean_dist(threshold_x_y_fitness_1, x_y_fitness_1).to(
                device
            )
        ...
        new_x_y_index_1 = scatter(
            x_y_index[torch.where(node_type == 1)], cluster_1, dim=0, reduce="mean"
        ).to(device)
        new_x_y_index = torch.cat(
            (torch.tensor([[0, 0]]).to(device), new_x_y_index_1), 0
        ).to(device)

        # fitness of third resolution-level node
        fitness_2 = (
            (x[torch.where(node_type == 2)] * self.weight_2).sum(dim=-1).to(device)
        )
        fitness_2 = self.nonlinearity(fitness_2 / self.weight_2.norm(p=2, dim=-1))

        # concat spatial information
        x_y_fitness_2 = torch.cat(
            (x_y_index[torch.where(node_type == 2)], fitness_2.unsqueeze(1)), -1
        )
        ...
        cluster2_from_1 = cluster_1[
            tree[torch.where(node_type == 2)]
            - torch.min(tree[torch.where(node_type == 2)])
        ]
        ...
        new_tree = torch.cat(
            (
                new_tree,
                torch.tensor([0] * len(set(cluster_1.cpu().numpy()))).to(device),
            ),
            0,
        ).to(device)
        ...
        for k in range(len(set(cluster_1.cpu().numpy()))):
            ...
            index_of_after_cluster = torch.where(
                cluster2_from_1
                == torch.tensor(sorted(list(set(cluster_1.cpu().numpy()))))[:, None][
                    k
                ].to(device)
            )
            ...
            after_cluster_x_y_fitness_2 = x_y_fitness_2[index_of_after_cluster].to(
                device
            )
            ...
            if self.select == "inter":
                if N_k < 4:
                    index_of_threshold_fitness_2 = sort_fitness_2_index[
                        random.sample(range(N_k), N_k)
                    ].to(device)
                else:
                    index_of_threshold_fitness_2 = sort_fitness_2_index[
                        random.sample(range(N_k), 4)
                    ].to(device)
            threshold_x_y_fitness_2 = after_cluster_x_y_fitness_2[
                index_of_threshold_fitness_2
            ].to(device)

            # Clustering according to Euclidean distance
            if self.dis == "ou":
                cosine_dis_2 = euclidean_dist(
                    threshold_x_y_fitness_2, after_cluster_x_y_fitness_2
                ).to(device)
            ...
            new_x_y_index = torch.cat(
                (
                    new_x_y_index,
                    scatter(t_x_y_index_2, t_cluster_2, dim=0, reduce="mean"),
                )
            ).to(device)
            ...
            new_tree = torch.cat(
                (
                    new_tree,
                    torch.tensor([k + 1] * len(set(t_cluster_2.cpu().numpy()))).to(
                        device
                    ),
                )
            )
        ...
        cluster = torch.where(
            cluster[:, None]
            == torch.tensor(sorted(list(set(cluster.cpu().numpy())))).to(device)
        )[-1].to(device)
